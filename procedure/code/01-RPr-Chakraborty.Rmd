...
packages <- c(
  ...
  "geepack", "knitr", "kableExtra", "foreign", "broom", "dotwhisker"
)
...
**Unplanned deviation for reproduction:**
Join the original author's Incidence data into our reproduction data frame so that we can later test for sensitivity to this error.
...
# check if there are any duplicated counties
cat("\n\nSaTScan Hierarchical clusters include", 
    length(satscan_rpr_hier$fips), 
    "records with",
    length(unique(satscan_rpr_hier$fips)),
    "unique county values")

cat("\n\nSaTScan GINI-optimized clusters include", 
...
- `rp_clusID` based on our SpatialEpi clusters 
...
Calculate GEE models with:
- Clustering: SaTScan cluster centers & State ID
- Dependent variable: original COVID-19 incidence (including errors).
...
Calculate GEE models with:
- Clustering: SaTScan cluster centers & State ID
- Dependent variable: reproduced COVID-19 incidence (fixed errors).
...
Calculate GEE models with:
- Clustering: Reproduced SaTScan GEE clusters & State ID
- Dependent variable: reproduced COVID-19 incidence (fixed errors).
...
Calculate GEE models with:
- Clustering: Reproduced SpatialEpi clusters & State ID
- Dependent variable: reproduced COVID-19 incidence (fixed errors).
...
Make Dotwhisker visualization of model results.

```{r dotwhisker-comparison}
geewhisker <- table2 %>% mutate(model = "Original Study") %>% select(-c(starts_with("row"), "wald_chi_square", "p_stars"))

geeterms <- table2 %>% select("term")

nextwhisker <- geeterms %>% 
  bind_cols(gee_or_clus_ordep$coefficients) %>% 
  select(1:5) %>% 
  mutate(model = "Switch to R Geepack")

geewhisker <- geewhisker %>% bind_rows(nextwhisker)

nextwhisker <- geeterms %>% 
  bind_cols(gee_or_clus_rpdep$coefficients) %>% 
  select(1:5) %>% 
  mutate(model = "Correct COVID Incidence")

geewhisker <- geewhisker %>% bind_rows(nextwhisker)

nextwhisker <- geeterms %>% 
  bind_cols(gee_gini_clus_rpdep$coefficients) %>% 
  select(1:5) %>% 
  mutate(model = "SaTScan GINI-Optimized Clusters")

geewhisker <- geewhisker %>% bind_rows(nextwhisker)

nextwhisker <- geeterms %>% 
  bind_cols(gee_rp_clus_rpdep$coefficients) %>% 
  select(1:5) %>% 
  mutate(model = "SpatialEpi Clusters")
...
```{r Reproduced-r-original-SPSS-diff}
(gee_or_clus_ordep$coefficients[, 1:6] - table2[, 3:ncol(table2)]) %>% 
  kable(caption = "Reproduced model minus Original model") %>% 
  kable_styling()
```
...
```{r incidence-difference}
(gee_or_clus_rpdep$coefficients - gee_or_clus_ordep$coefficients) %>% 
  kable(caption = "Reproduced model minus Original model") %>% 
  kable_styling()
```

First, here are two notes about interpreting the difference between models:
- A positive number for the difference of coefficients indicates that the reproduction found a greater coefficient.
- A positive number for the p-value indicates that the reproduction found a larger p-value, for a *less statistically signficant* result.

As a result of fixing the COVID-19 incidence rate errors, the significance level of one variable (Asian people with disabilities) decreased.
The statistical significance of the Age 35-64 group increased by 0.08, but the variable remained above the 0.05 significance level.

Overall, the impact of errors in COVID-19 incidence rates for 13 counties appears to have been minimial in the context of an overall sample size of 3108 counties.
...
tmap_save(tm_author_clusters, here("results", "figures", "author_clusters.png"))
