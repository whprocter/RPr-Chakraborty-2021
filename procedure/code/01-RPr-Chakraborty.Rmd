...
packages <- c(
  ...
  "geepack", "knitr", "kableExtra", "foreign"
)
...
author_clusters <- acs_covid %>% 
  ...
  mutate(GINI_CLUST = ifelse(GINI_CLUST == 'T', 'Optimized GINI', 'Hierarchical'))
...
tm_author_clusters <-
  ...
  tm_add_legend('symbol', 
              	...
              	size = 0.7,
              	labels = c('Hierarchical','Optimized GINI'),
...
tm_spatialepi_satscan <-  
    tm_shape(state) +
      tm_fill("gray98") +
    tm_shape(acs_covid_clusters) +
    tm_polygons(col = "xcluster", 
            palette = c("wheat","tomato", "thistle3"),
            colorNA = NULL,
            title = "SaTScan and SpatialEpi Clusters",
            border.col = "white",
            lwd = 0.2,
            border.alpha = 0.3) +
  tm_shape(state) +
...
Third, the geographic patterns and cluster relative risk scores of secondary clusters are very sensitive to changes in data and to subjective parameters.
...
table(ss_se$se_class, ss_se$ss_class, dnn=c("SpatialEpi", "SaTScan")) %>% 
  ...
  add_header_above(c("SpatialEpi" = 1, "SatScan" = 3)) 
