...
**Planned deviation for reproduction**: We also map the spatial distribution of the percent of people with any disability.
...
Correlation difference `rp_r_diff` is calculated between the reproduction study `rp_r` and original study `or_r` as `rp_r_diff = rp_r - or_r`
Direction difference `rp_dir_diff` is calculated as `(rp_r > 0) - (or_r > 0)`, giving `0` if both coefficients have the same direction, `1` if the reproduction is positive and the original is negative, and `-1` if the reproduction is negative but the original is positive.
...
We opted to use the SpatialEpi package in R, selecting open source software with R integration over SatSCan software, which is free but not open.
...
The original study does not include a visualization of the spatial distribution of COVID-19 clusters.
...
**Unplanned deviation for reproduction**:
The `SpatialEpi` implementation of Kulldorff spatial scan statistics does not calculate cluster relative risk.
...
Note that this version of data only includes the 96 counties defining cluster centers, excluding all of the counties in a cluster, but not at the cluster's center.

Additionally, the SaTScan software confusingly merges two sets of clusters in the results when the user uses the (default) option for GINI-optimized clusters, and the two sets may overlap geographically even if the non-geographic overlap option is used.
One set of results is a hierarchical non-overlapping set of clusters determined by iteratively selecting the secondary cluster with the greatest log-likelihood ratio that does not overlap any selected clusters, from a set of clusters with the maximum log-likelihood ratio at each location.
These clusters are noted with `GINI_CLUST = F` in the results.

The second set of results is found by finding numerous hierarchical non-overlapping sets of clusters based on the method above, but each with the constraint of a different maximum cluster size.
The set of clusters with the highest GINI ratio is selected.
These clusters are noted with `GINI_CLUST = T` in the results.

Taken together as they are, the two sets of secondary clusters may overlap geographically, causing ambiguity in terms of which cluster-based relative risk score should be used at each location.

**Unplanned Deviation for Reanalysis:**
If we load the local SaTSCan results inclusive of all counties in a clusters and filter the results to focus on the standard hierarchical set of clusters, is there consistency between the SaTScan and SpatialEpi results?
...
tm_spatialepi_satscan <-  
  tm_shape(acs_covid) +
    tm_fill(col = "xcluster", 
            palette = c("wheat","tomato", "palegreen2"),
            colorNA = NULL,
...
least one source of difference in differences in GEE clusters may be differences in Kulldorff spatial scan statistic calculations between SaTScan and SpatialEpi is both differences in user decisions and options and logical implementation of the selection of secondary clusters.
