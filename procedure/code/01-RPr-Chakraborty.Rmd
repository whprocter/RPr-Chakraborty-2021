...
*Unplanned Deviation* Initially the descriptive statistics for the COVID rate were slightly different, but we have resolved the difference by rounding the rate to 0 decimal places.
...
**Planned deviation**: We also map the spatial distribution of the percent of people with any disability.
...
Difference is calculated as 'reproduction study - original study' Identical results will result in zero.
...
correlations <- correlations %>% 
  bind_cols(rename_with(correlations[,4:5] - correlations[,2:3],
                                                      ~ paste0(.x, "_diff")))
...
The statistic uses a Monte Carlo simulation with 999 iterations.
Therefore, the code may not produce identical results each time.
...
```{r report Kulldorff results}
print("Most likely cluster:", quote = FALSE)
covid_kulldorff$most.likely.cluster
print(paste0("Number of Secondary clusters: ", 
             length(covid_kulldorff$secondary.clusters)), 
      quote = FALSE)
```

The `SpatialEpi` implementation of Kulldorff spatial scan statistics provides output in the form of hierarchical lists analogous to the text output of SaTScan, but does not ouput a data frame.
...
The default SatSCan setting is to find only non-overlapping clusters, such that each county is part of at most one cluster.
SpatialEpi is also implemented to find only non-overlapping clusters.
...
rm(new_clusters, cluster_locations, clusters)

tmap_mode("view")
tm_basemap("OpenStreetMap.HOT") +
  tm_shape(acs_covid) + 
  tm_fill(alpha=0.7, col="isCluster", palette="-Oranges", popup.vars=c("fips","clusterID"))
```

The SaTScan software implementation of the Kulldorff spatial scan statistic calculates two relative risk scores:
...
For the purposes of interpreting the spatial scan statistic, a *location* is a *county* an a *cluster* is a *collection of counties* with high incidence rates, defined in the shape of a circle with a center and a radius.
...
Therefore, the next step is to calculate local relative risk (workflow step 7).
...
**Unplanned deviation**: Also calculate the relative risk score of each cluster so that we can test whether cluster-based risk classifications are better controls for the spatial/regional heterogeneity of COVID-19 risk.

```{r cluster relative risk}
# Calculate cluster-based relative risk for counties in a cluster
acs_covid <- acs_covid %>% 
  group_by(clusterID) %>%
  mutate(
...
Reclassify relative risk on a scale from 1 to 6 (workflow step 8).
...
Summarize results of the local county-based relative risk classification.

```{r local classification results}
# Count frequency of each class of local COVID risk
cat("Classes of local risk and frequency of counties",
  format(acs_covid %>% st_drop_geometry %>% count(loc_class)), sep="\n")
...
Summarize results of the cluster-based relative risk classification.

```{r cluster classification results}
cat("Classes of cluster risk and frequency of counties",
  format(acs_covid %>% st_drop_geometry %>% count(cluster_class)), sep="\n")
# better to use kable() for these outputs? Check knit results...
```

Create unique cluster IDs as combinations of the relative risk class and the state.
...
cluster_summary <- acs_covid %>% filter(cases > 0) %>% 
  st_drop_geometry() %>%
  count(locID)
cat(length(cluster_summary$n), 
...
**Unplanned deviation**: Upon failing to reproduce an identical number of GEE clusters using SpatialEpi in R, we reproduced the procedure in the free but not open SaTScan software.
The input data files (case, Coordinates.geo, and Population.pop), parameter file (sat_scan_runfile.prm), and output data files (sat_scan_compare.txt, sat_scan_compare.col.shp, and sat_scan_compare.gis.shp) are found in the `data/derived/public/satscan` directory.

We viewed the SatSCAN outputs in QGIS Desktop and discovered that the spatial ouptut includes two shapefiles: 

- col cluster polygon shapefile containing a circle for each cluster, where each polygon is a circle defined by the cluster center and radius. Contains variable `REL_RISK` for relative risk **(LOCAL OR CLUSTER RISK???)**
- gis location point shapefile contains one point for each county in a cluster. Contains variables `LOC_RR` for local relative risk  and `CLU_RR` for cluster relative risk

Considering that the author-provided `Aug1GEEdata.csv` data contains a `REL_RISK` column, and author-provided `SatScan_results.txt` indicates a geographic cluster file but no location file, we can expect that the original study used the `col` polygon cluster shapefile representing the centers of each cluster.

Load and verify the SatSCan reproduction results.

**NOTE** Unfortunately, our satscan results appear to have geographic overlap even though the text output does not. Redo?

```{r load and process satscan reproduction}
satscan_col <- read_sf(here("data", "derived", "public", "satscan", "sat_scan_compare.col.shp"))
satscan_col_t <- satscan_col %>% st_drop_geometry()
satscan_gis <- read_sf(here("data", "derived", "public", "satscan", "sat_scan_compare.gis.shp")) %>% st_drop_geometry()
