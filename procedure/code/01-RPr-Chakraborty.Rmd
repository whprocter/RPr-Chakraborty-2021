...
The original study does not include a visualization of the spatial structure and distribution of COVID-19 clusters.
Therefore, calculate a new field `isCluster` to identify counties in COVID-19 clusters.
...
Then, find the number of unique clusters and frequency counties per cluster in our reproduction study for comparison to the original study.
...
There are stark differences between the two sets of clusters, with the agreement only for the largest three SaTScan clusters.
...
There is more agreement between SpatialEpi and SaTScan GINI Optmization for smaller clusters, but the SaTScan clusters are limited in size, resulting in several smaller clusters with gaps in place of the largest SpatialEpi clusters.
...
We can check by calculating cluster relative risk classes based on the SaTScan GINI clusters, and crosstabulating with the SpatialEpi risk classes.

```{r compare-spatialepi-satscan}
acs_covid <- acs_covid %>%
  mutate(gini_class = ifelse(
    is.na(gini_rr),
    1,
    cut(gini_rr, breaks, labels = FALSE)
  ))

table(acs_covid$cluster_class, acs_covid$gini_class, dnn = c("SpatialEpi", "SaTScan")) %>%
...
Second, two sets of overlapping secondary clusters are included in the SaTScan output: hierarchical clusters and clusters optimized by GINI coefficient.
Third, the geographic patterns and cluster relative risk scores of secondary clusters are limited to circles or ellipses and are very sensitive to subjective parameters.
