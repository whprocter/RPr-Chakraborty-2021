...
Version 1.3 | Created Jul 7, 2021 | Last Updated June 21, 2022
...
clusters <- covid_kulldorff$most.likely.cluster$location.IDs.included 
# NEED TO DO: transform clusters into a dataframe with two columns: clusterID and countyID
...
for (i in secondary) {
  # NEED TO DO: transform each list of countyIDs into a dataframe with two columns: clusterID and countyID, before appending those rows to clusters
  clusters <- c(clusters, i$location.IDs.included)
}

# NEED TO DO: calculate relative risk for each cluster (rr_cluster), using dyplr to group by clusterID; then ungroup and group by COUNTYID to summarize the greatest relative risk for each county
...
# NEED TO DO: calculate rr_class using newly caculated rr_cluster rather than rr_loc
