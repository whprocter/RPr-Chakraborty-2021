...
### Function to call one GEE model

```{r gee-functions}
onegee <- function(gee_data, ind_vars, dep_var, id) {
  gee_data <- gee_data %>% arrange({{ id }})
  geemodel <- geeglm(
    formula = as.formula(paste(dep_var, "~", ind_vars)),
    data = gee_data,
    id = {{ id }}, # cluster IDs
...
racemodel <- onegee(gee_data, 
                    ind_vars = "z_white_pct + z_black_pct + z_native_pct + z_asian_pct + z_other_pct", 
                    dep_var = "covid_rate",
                    id = statefp)
```


#### Use functions to do this so we can iterate over study parameters
