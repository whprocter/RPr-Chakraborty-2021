...
## Kulldorff Spatial Scan Cluster Detection

We use a Kulldorff spatial scan statistic  
accomplishes the step 6 of the workflow diagram

The Kulldorf spatial scan uses a Monte Carlo simulation with 999 iterations.
Therefore, the code can take several minutes to run and may not produce identical results each time.
If you wish to exactly reproduce the same results as our reproduction attempt, please **do not run this section**.
Instead, load the scan results in the "
...
```
**Unplanned deviation**: 
The `SpatialEpi` implementation of Kulldorff spatial scan statistics provides output in the form of a collection of data frames, one for each cluster.
Therefore, we need still to process the Kulldorff results to calculate local relative risk, cluster ID's, and cluster relative risk for each county.

First, calculate the local relative risk for each county (step 7 of the workflow diagram).
...
Second, assign unique cluster ID's to each county within a cluster.
Clusters include the county at the center of a cluster and all of the other counties within the cluster radius.
`SpatialEpi` finds only non-overlapping clusters, such that each county is part of at most one cluster.

```{r assign cluster IDs}
...
# join clusterID to acs_covid geodataframe
acs_covid <- acs_covid[,1:29] %>% left_join(clusters, by = "fips")

rm(new_clusters, cluster_locations, clusters)
```

Calculate the relative risk score of each cluster (step 7 of workflow diagram).
...
acs_covid <- acs_covid %>% 
  ...
  mutate(
    rr_cluster = ifelse(is.na(clusterID), NA, 
      (sum(cases) / sum(pop)) / 
        ((total_cases - sum(cases)) / (total_pop - sum(pop))))) %>% 
  ungroup()
```

Reclassify relative risk on a scale from 1 to 6 (step 8 of the workflow diagram).
Counties falling outside of any cluster are assigned a score of 1.

```{r classify local relative risk}
# class breaks
...
Need to report number of unique clusters after combining risk class with state ID.
