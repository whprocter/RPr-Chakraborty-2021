...
correlations %>%
  ...
  kable() %>% column_spec(2:6, width_min = "5em")
```
...
**Unplanned deviation for reproduction**: 
Now that we have calculated the Kulldorff spatial clusters and their local and relative risk scores, it would be helpful to visualize the spatial distributions in advance of using these classes to control for spatial heterogeneity in GEE models.
...
First, calculate any additional GEE cluster IDs that may be required.
...
```{r geeclusters-arrange}
# calculate GEE cluster IDs and sort by the IDs (a requirement of the gee function)
acs_covid <- acs_covid %>% 
...
```{r gee-function}
# function for calculating GEE models
# gee_data is a data frame of input data
# gee_id is a string naming the column with unique GEE cluster IDs
# covid_rate is a string naming the column with outcome y-variable (i.e. the COVID rate)

gee_rpr <- function(gee_data, id, covid_rate = "covid_rate") {
  
  gee_data <- gee_data %>% 
    st_drop_geometry() %>% 
    mutate(gee_id = gee_data$id
  
  # sort data by GEE cluster ID
  gee_data <- gee_data %>% arrange(gee_id)
  
  # try using as.formula() function to build the formula from strings
  
  # race model
  race_gee <- geeglm(
    covid_rate ~ z_white_pct + z_black_pct + z_native_pct + z_asian_pct + z_other_pct,
    data = gee_data, # data frame
    id = gee_id, # cluster IDs
