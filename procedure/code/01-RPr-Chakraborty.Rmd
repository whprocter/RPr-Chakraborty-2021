...
This is a test to see if everything syncs.

What am I doing?

Who knows.

Experimenting with a different possible research decision, and evaluating how that changes the outcome

Identify a part of the study where it would be good to have another map or visualization, and interpreting it

Catch any errors and fix them?

Is any of the code inefficient?
...
However, we replace the missing data with zeros, producing results identical to Chakraborty's. **WHAT IS THS JUSTIFICATION FOR THIS? WHY NOT REMOVE THE COUNTY?**


```{r missing data}
# county with missing data
filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% kable()           #Identify the county with N/A data     #What does kable() do??????
...
The descriptive statistics are identical, except that the original study seems to have rounded the COVID-19 statistics to zero decimal places. **Great point here**
...
This finding implies that subsequent analyses using the COVID-19 Incidence rate will be slightly different and more accurate in this reproduction study than in the original study.  **WOW, CRAZY THAT THE INCIDENCE SCORES WERE INCORRECT.  HOW DID YOU THINK TO CHECK?**
