...
gee_resid <- data.frame(
  ...
  mutate(
    my_response = covid_rate - fitvals,
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = my_response / sqrt(fitvals)
  ) %>%
  st_sf()

# response residual is: y - f(x)
# the model residual is identical to working residual
# my calculation of what I thought was the working residual (response / fitted value) is identical the Pearson's residual
# resource on residuals of GLM models in R: https://www.datascienceblog.net/post/machine-learning/interpreting_generalized_linear_models/
```
...
Calculate variance, covariance, corrrelation, and weights
...
grps <- gee_resid %>% st_drop_geometry() %>% select(ss_clusID, residual)
...
outer(grps$ss_clusID, grps$ss_clusID, "==")[80:90,80:90] - 
  outer(rownames(grps), rownames(grps), "==")[80:90,80:90]

for (x in 1:(n-1)){
  for(y in (x+1):n){
    if(grps[x,"ss_clusID"] == grps[y,"ss_clusID"]){
      m[x,y] <- 1
    }
  }
}
...
# variance
v <- sum(gee_resid$residual^2) / (n - 1)
# covariance
