...
```{r Load ACS Data, message = FALSE, data_acquisition}
# get API Key
...
```{r filter and join acs data, suppress}
...
*Unplanned Deviation for Reproduction*: We re-calculate the COVID-19 incidence rate with the original author's data and select for unequal results.
...
Some instabilty may also be attributable to the 13 counties with data errors in the COVID-19 Incidence Rate.
...
clusters <- covid_table[cluster_locations, "fips"] %>%
  ...
  mutate(likelihood = covid_kulldorff$most.likely.cluster$log.likelihood.ratio)
...
rm(covid_kulldorff, cluster_locations, secondary, i, new_clusters)
```
...
**Unplanned deviation for reproduction**:
Can we also exactly reproduce GEE classifications with our SaTScan results?
...
**Can we explain the differences here?** SaTScan is sequentially adding non-overlapping secondary clusters with the highest log-likelihood. What is SpatialEpi doing?
