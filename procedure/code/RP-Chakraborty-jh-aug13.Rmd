...
# There's a relative risk score for both county & cluster (in SatScan); our analysis uses county based on example in paper (paragraph 4 of methods) & citation Desjardins et al
```
...
gee_data <- filter(acs, !is.na(bpov_pct) & !is.na(apov_pct) & covid_rate >0)

gee_data <- mutate(gee_data, id=as.integer(statefp) * 10 + rr_class)

race_gee <- gee(covid_rate ~ white_pct + black_pct + native_pct + asian_pct + other_pct, # regression formula, dependent var first, then sequence of independent variables
...
ethnicity_gee <- gee(covid_rate ~ non_hisp_pct + non_hisp_non_white_pct + hisp_pct, # regression formula, dependent var first, then sequence of independent variables
               data = gee_data, # data frame
               id = id, # cluster IDs
               family = Gamma(link = "log"), # "gamma distribution with logarithmic link function"
               corstr = "exchangeable" # use exchangeable correlation matrix
...
age_gee <- gee(covid_rate ~ pct_5_17 + pct_18_34 + pct_35_64 + pct_65_74 + pct_75, # regression formula, dependent var first, then sequence of independent variables
               data = gee_data, # data frame
               id = id, # cluster IDs
               family = Gamma(link = "log"), # "gamma distribution with logarithmic link function"
               corstr = "exchangeable" # use exchangeable correlation matrix
...
sex_gee <- gee(covid_rate ~ male_pct + female_pct, # regression formula, dependent var first, then sequence of independent variables
               data = gee_data, # data frame
               id = id, # cluster IDs
               family = Gamma(link = "log"), # "gamma distribution with logarithmic link function"
               corstr = "exchangeable" # use exchangeable correlation matrix
