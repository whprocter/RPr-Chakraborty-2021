...
gee_data <- acs %>%
  select(contains("pct"), contains("rate")) %>%
...
temp <- acs[!is.na(acs$bpov_pct), ]
gee_data <- temp %>%
         mutate(
         bpov_z = (bpov_pct - mean(bpov_pct) / sd(bpov_pct)),
         apov_z = (apov_pct - mean(apov_pct) / sd(apov_pct))
         )
...
covid <- covid[covid$FIPS != "35039", ]

age_gee <- gee(covid_rate ~ gee_data$bpov_z + gee_data$apov_z, # regression formula, dependent var first, then sequence of independent variables
