...
acs_covid <- select(acs_covid, geoid, statefp, county, county_st, covid_rate, everything())
```
...
acs_covid_stats <- acs_covid %>% 
  ...
  round(2) %>% 
  t() %>% as.data.frame() %>% 
  select(min, max, mean, SD = std.dev, ShapiroWilk = normtest.W, sw_p = normtest.p )
...
This non-parametric correlation test is better for t
...
paste(sum(covid$cluster==0 & covid$rr_loc >= 1),
...
# if I'm doing this, I might a well be renaming them all.
